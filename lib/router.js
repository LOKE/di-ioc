...
class Router {
  ...
  get(name) {
    ...
    const thisName = nameArr.shift();

    if (nameArr.length) {
      return this.get(thisName).factory.get(nameArr.join('.'));
    }
  ...
  update(name, patch) {
    ...
    const def = router.get(lName);
    router.unset(lName);
    router.add(Object.assign({}, def, patch));
  }
  ...
  _build(getters, prefix) {
    ...
    defs.forEach((def) => {
      ...
      if (def.type === TYPE_SUBROUTER) {
        ...
        container.set(def.name, subcontainer);
        container.setActive(subcontainer);
        return;
      }
